...
app.post('/create_repository', (req, res) => {
    ...
    // Validate Post Data
    const data = req.body;
    if(!data.git_name || typeof data.git_name !== 'string' || data.git_name.length == 0 || hasSpace(data.git_name)) {
        // Invalid Data
        obj.err = "Invalid JSON Data, please provide { git_name: 'string' } with data. Git name cannot have spaces!";
        res.send(obj);
        return;
    }
    
    // Run script to create new Repository
    const script = spawn('sh', ['/home/git/scripts/create_new_repository.sh', data.git_name ]);
    ...
    script.on('close', code => {
        ...
        }else if (code === 2) { // ERROR: Duplicate Repository
...
app.get('/list_repositories', (req, res) => {
    ...
    script.on('close', code => {
        // Check Script Status Code
        if(code === 0) {        // No Errors
        }else if (code === 2) { // ERROR: Repository not Found
...
app.post('/push_key', (req, res) => {
    ...
    if(data && data.key) {
        ...
        const script = spawn('sh', ['/home/git/scripts/add_public_sshkey.sh', data.key ]);
...
const server = app.listen(3000, (s) => {
    console.log(`ExpressJS Server Listening on ${server.address().address}: ${server.address().port}`);
});
